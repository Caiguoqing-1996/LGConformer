...
if __name__ == "__main__":
    ...
    excel_path = 'BCI_2a_STformer_CrossSession.xlsx'
    model_sel_all = ['MSConformer', 'MSConformer']

    df = pd.DataFrame(columns=['model_sel', 'n_layer', 'spa_dim', 'acc_all', 'mean_acc', 'best_acc_all', 'best_mean_acc'])
    ...
    for model_sel, spa_dim, n_layer in itertools.product(model_sel_all, spa_dim_all, n_layer_all):
        ...
        all_acc_test, best_model, all_precision, all_f1, all_kappa \
            = CrossValidation_BCI4_2a(data_sel, model_sel, model_para, train_para)

        torch.save(best_model, 'LGConformer_BCI_bestmodels.pth')
